...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			...
			// 模板方法用于初始化FrameworkServlet，该方法并未使用
	...
	protected WebApplicationContext initWebApplicationContext() {
		// 获取spring的根容器rootContext，详细可参考：https://sayno.win/?p=271
		WebApplicationContext rootContext =
				WebApplicationContextUtils.getWebApplicationContext(getServletContext());
		WebApplicationContext wac = null;

		// 设置webApplicationContext一共三种方法
		// 方法一：构造方法中传入webApplicationContext参数（Servlet3.0以后的环境使用）
		...
		// 方法二：webApplicationContext已经在ServletContext中了，这时只需要配置Servlet的
		// webApplicationContext的name配置到contextAttribute属性就可以了。
		if (wac == null) {
		...
		// 方法三：前面两种都无效的情况下，自己创建一个webApplicationContext。正常情况下都是使用的这
		// 种方式。
		if (wac == null) {
			// No context instance is defined for this servlet -> create a local one
			// 传入spring根容器（rootContext），创建webApplicationContext
			wac = createWebApplicationContext(rootContext);
		}

		// 根据refreshEventReceived判断webApplicationContext是否已经刷新过，方法一和方法三种已经刷新过
		// webApplicationContext，则不需要再次刷新
	...
	protected WebApplicationContext findWebApplicationContext() {
		// DispatcherServlet中配置contextAttribute参数，然后从ServletContext根据
		// attribute的name获取webContextApplication
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		// 1.获取创建的类型，可以通过contextClass配置，配置项配置实现
		// org.springframework.web.context.ConfigurableWebApplicationContext接口，
		// 默认使用org.springframework.web.context.support.XmlWebApplicationContext
		...
		// 2.类型校验，是否实现ConfigurableWebApplicationContext接口
		...
		// 3.通过BeanUtils创建webApplicationContext
		ConfigurableWebApplicationContext wac =
				(ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass);
		// 4.设置环境参数
		wac.setEnvironment(getEnvironment());
		// 设置rootApplicationContext
		wac.setParent(parent);
		// 设置contextConfigLocation参数，如果web.xml中未配置，默认使用【servletName】-Servlet.xml
		String configLocation = getContextConfigLocation();
		if (configLocation != null) {
			wac.setConfigLocation(configLocation);
		}
		// 5.配置并刷新WebApplicationContext
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// WebApplicationContext绑定ServletContext、ServletConfig以及nameSpace
		wac.setServletContext(getServletContext());
		wac.setServletConfig(getServletConfig());
		wac.setNamespace(getNamespace());
		
		// SourceFilteringListener可以根据输入的参数进行选择，实际监听的是ContextRefreshListener所监听的事件，
		// ContextRefreshListener是FrameworkServlet内部类，监听ContextRefreshedEvent事件，当接到消息时
		// 调用FrameworkServlet的onApplicationEvent方法，调用onRefresh方法，并设置refreshEventReceived
		// 为true代表容器已经refresh。
		wac.addApplicationListener(new SourceFilteringListener(wac, new ContextRefreshListener()));

		// environment包括五个属性:
		// 1.ServetConfigProperty
		// 2.ServetContextProperty
		// 3.JndiProperty
		// 4.MapProperty(虚拟机属性)
		// 5.SystemEnvironment(系统环境，如环境变量JAVA_HOME，Path等信息)
		...
		// 模板方法，子类可以覆盖该方法，完成容器初始化之前的操作，此处为空方法
		postProcessWebApplicationContext(wac);
		// 执行webApplicationContext初始化
		applyInitializers(wac);
		// 调用webApplicationContext的refresh方法，刷新spring容器
		wac.refresh();
	}
	...
	protected void applyInitializers(ConfigurableApplicationContext wac) {
		// 通常在web应用程序中需要对应用程序上下文做一些程序初始化时，例如：注册property资源文件或者激活
		// 对上下文环境的环境配置文件分别通过在<context-param>和<init-param>中声明
		// contextInitializerClasses参数来的支持。 
		// 配置的参数要实现ApplicationContextInitializer接口
		// 非必须不建议使用
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 获取LocaleContextHolder中保存的LocaleContext
		...
		// 将当前请求的localeContext，requestAttributes设置到LocaleContextHolder和RequestContextHolder
		initContextHolders(request, localeContext, requestAttributes);

		try {
			// 处理出口，模板方法，在DispatcherServlet中具体处理
			...
			// 恢复原来的previousLocaleContext和previousAttributes
