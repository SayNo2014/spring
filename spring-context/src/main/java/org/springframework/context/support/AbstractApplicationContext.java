...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		refreshBeanFactory();
		// 获取BeanFactory,其实getBeanFactory方法获取的是refreshBeanFactory中创建的
		// BeanFactory
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 增加默认属性编辑器
		// resourceLoader:xml
		// this:Root WebApplicationContext(XmlWebApplicationContext)
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 调用的BeanFactory的后置处理器
		// getBeanFactoryPostProcessors#beanFactoryPostProcessors全部为Spring硬编码创建的Bean后置处理器
		PostProcessorRegistrationDelegate.invokeBeanFactoryPostProcessors(beanFactory, getBeanFactoryPostProcessors());

		// 添加了AOP支持,此处暂且不做分析,后面再分析AOP过程中详解
	...
	protected void initApplicationEventMulticaster() {
		...
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
			// 保存名称为 applicationEventMulticaster 的 bean
			...
			// 若不存在,则创建SimpleApplicationEventMulticaster
...
//	        }); 
	...
	protected void finishRefresh() {
		// 清空context级别的resource caches（例如ASM）
		clearResourceCaches();

		// 初始化lifecycle processor
		initLifecycleProcessor();

		// 调用lifecycle processor的onRefresh方法
