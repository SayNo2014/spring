...
public abstract class AbstractRefreshableWebApplicationContext extends AbstractRefreshableConfigApplicationContext
	...
	protected void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		// 添加Bean后置处理器ServletContextAwareProcessor,
		// 作用是：如果XXBean实现了ServletContextAware和ServletConfigAware会将ServletContext和ServletConfig
		// 传递给XX
		beanFactory.addBeanPostProcessor(new ServletContextAwareProcessor(
				this.servletContext, this.servletConfig));
		// 设置ServletContextAware和ServletConfigAware在依赖检查和自动绑定时为要忽略的依赖接口
		// 默认情况下，只有beanFactory接口被忽略。
		beanFactory.ignoreDependencyInterface(ServletContextAware.class);
		beanFactory.ignoreDependencyInterface(ServletConfigAware.class);

		// 创建了requset、session、application的Scope对象,并设置了自动装配规则
		WebApplicationContextUtils.registerWebApplicationScopes(
				beanFactory, this.servletContext);
		// 将servletContext,servletConfig,contextParameters,contextAttributes
		// 保存到XmlWebApplicationContext中
		// 此处都是通过registerSingleton注册的单例,详细会在创建Bean过程中详解该方法
