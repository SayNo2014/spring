...
public class ContextLoader {
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// 3. 配置并刷新WebApplicationContext
					configureAndRefreshWebApplicationContext(cwac, servletContext);
				}
			}
			
			// 将初始化后的context保存在servletContext
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		...
		// 获取类无参构造函数,然后创建WebApplicationContext
		return (ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass);
	}

	protected Class<?> determineContextClass(ServletContext servletContext) {
		// 1. 获取web.xml中配置的contextClass
		...
		// 2. 如果web.xml中未配置contextClass,则使用默认的WebApplicationContext作为contextClassName
		// 	      默认的配置为文件ContextLoader.properties配置的org.springframework.web.context.WebApplicationContext
		else {
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
			// 从ServletContext获取contextId,若未配置使用默认方法生成
		...
		// WebApplicationContext绑定ServletContext
		wac.setServletContext(sc);
		// 获取web.xml中指定的contextConfigLocation配置文件的位置
		String configLocationParam = sc.getInitParameter(CONFIG_LOCATION_PARAM);
		if (configLocationParam != null) {
			wac.setConfigLocation(configLocationParam);
		}

		// 获取Environment实例,若不存在创建
		ConfigurableEnvironment env = wac.getEnvironment();
		if (env instanceof ConfigurableWebEnvironment) {
			// 将ServletContext属性放置到env内
			((ConfigurableWebEnvironment) env).initPropertySources(sc, null);
		}

		// 获取web.xml用户自定义的初始化类,初始化wac
		customizeContext(sc, wac);
		// org.springframework.context.support.AbstractApplicationContext#refresh容器刷新
		// 下一篇博文分析该方法
		wac.refresh();
	}
	...
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		...
		// 校验配置的类是否合格,不合格抛出异常,若合格则添加到contextInitializers中备份
		...
		// 调用其initialize方法初始化WebXmlApplicationContext
	...
	protected List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>>
		...
		List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>> classes =
				new ArrayList<>();
		// 获取web.xml中配置globalInitializerClasses,返回其Class
		// 可以配置多个用INIT_PARAM_DELIMITERS进行分割
		...
		// 获取web.xml中配置contextInitializerClasses,调用其initialize方法初始化WebXmlApplicationContext
		// 可以配置多个用INIT_PARAM_DELIMITERS进行分割
		// 和globalInitializerClasses类似,只是作用域不同
