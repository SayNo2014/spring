...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	public Class<?> resolveBeanClass(@Nullable ClassLoader classLoader) throws ClassNotFoundException {
		...
		// ClassUtils#forName之前已经使用过,大概流程就是如果classLoader已经加载过该类,直接返回Class
		// 若未加载则调用Class.forName尝试加载
		Class<?> resolvedClass = ClassUtils.forName(className, classLoader);
		// 将加载后的Class实例保存到BeanDefinition#beanClass
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		// 获取对应类中对应方法名的个数
		// 我们自定Spring的bean标签中并没有override属性
		// 但是是否还有印象在博文【Spring 源码分析 – Spring默认标签解析流程】分析默认标签的look-up replace-method时有如下代码
        // 解析lookup-method
        // parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
        // 解析replaced-method
        // parseReplacedMethodSubElements(ele, bd.getMethodOverrides());
		// 这里所说的overrideMethod其实就是配置的look-up和replace-method
