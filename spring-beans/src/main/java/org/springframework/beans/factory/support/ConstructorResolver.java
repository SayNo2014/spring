...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(final String beanName, final RootBeanDefinition mbd,
		...
		// 1. 构造函数参数的确定
		// chosenCtors=null,explicitArgs=null
		...
		// 如果getBean方法调用的时候指定方法参数那么就可以直接使用
		if (explicitArgs != null) {
			...
			if (argsToResolve != null) {
				// 解析参数类型，如给定方法的构造函数A(int,int)则通过此方法后就会把配置中的A("1","1")转成A(1,1)
				// 缓存中的值可能是原始值也可能是最终值
		...
		if (constructorToUse == null) {
			...
			for (Constructor<?> candidate : candidates) {
				...
				// 如果已经找到构造函数或者需要的参数个数已经小于当前的构造函数参数个数则终止
				// 构造函数是按照参数数量降序排序的
				...
				// 4. 构造函数的不确定性校验
				// 是否有不确定性的构造函数存在,例如不同构造函数的参数为父子关系
